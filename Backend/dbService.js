...
class DbService{
  ...
  async searchBySalaryRange(min, max) {
    try {
      const query = "SELECT * FROM names WHERE salary BETWEEN ? AND ?;";
      const results = await new Promise((resolve, reject) => {
        connection.query(query, [min, max], (err, res) => {
          if (err) reject(err);
          else resolve(res);
        });
      });
  ...
  async searchByAgeRange(min, max) {
    try {
      const query = "SELECT * FROM names WHERE age BETWEEN ? AND ?;";
      const results = await new Promise((resolve, reject) => {
        connection.query(query, [min, max], (err, res) => {
          if (err) reject(err);
          else resolve(res);
  ...
  async searchNeverSignedIn() {
    try {
      const query = "SELECT * FROM names WHERE signInTime IS NULL;";
      const results = await new Promise((resolve, reject) => {
